...
export const GeocodeOriginBody = zod.object({
  "query": zod.string().min(2).max(120)
});
...
export const RecommendFromTagsBody = zod.object({
  ...
  "sessionId": zod.string().min(8).max(128).regex(/^[A-Za-z0-9._:-]+$/).optional(),
  "rerollOfRequestId": zod.string().min(8).max(128).regex(/^[A-Za-z0-9._:-]+$/).optional(),
...
export const RecommendFromTagsResponse = zod.object({
  ...
  "originStatus": zod.enum(['APPLIED', 'SKIPPED', 'UNAVAILABLE']).optional(),
