...
export const placesTable = pgTable(
  "places",
  {
    ...
    primaryTheme: text("primary_theme").notNull(),
    moodTags: jsonb("mood_tags").$type<string[]>().notNull(),
    sceneTags: jsonb("scene_tags").$type<string[]>().notNull(),
    photoPoint: text("photo_point").notNull(),
    purposeTags: jsonb("purpose_tags").$type<string[]>().notNull(),
    seasonTags: jsonb("season_tags").$type<string[]>().notNull(),
    bestTime: text("best_time").notNull(),
    accessibility: jsonb("accessibility").$type<Record<string, unknown>>().notNull(),
    recommendationUse: text("recommendation_use").notNull(),
    ...
    verificationItems: jsonb("verification_items").$type<unknown>(),
    sourceUrls: jsonb("source_urls").$type<string[]>(),
    sourceCheckedAt: timestamp("source_checked_at", { withTimezone: true }),
    operatingCondition: jsonb("operating_condition").$type<Record<string, unknown>>(),
    externalIds: jsonb("external_ids").$type<Record<string, unknown>>().notNull().default({}),
    isRecommendationCandidate: boolean("is_recommendation_candidate").notNull().default(true),
    isCourseCandidate: boolean("is_course_candidate").notNull().default(true),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => ({
    cityIdx: index("places_city_idx").on(table.city),
    themeIdx: index("places_primary_theme_idx").on(table.primaryTheme),
    recommendationIdx: index("places_recommendation_candidate_idx").on(table.isRecommendationCandidate),
    courseIdx: index("places_course_candidate_idx").on(table.isCourseCandidate),
  }),
).enableRLS();

export const placePhotosTable = pgTable(
  "place_photos",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    placeId: text("place_id").notNull().references(() => placesTable.placeId, { onDelete: "cascade" }),
  ...
  (table) => ({
    placeIdx: index("place_photos_place_idx").on(table.placeId, table.isActive),
    sourceIdx: uniqueIndex("place_photos_place_provider_source_idx").on(table.placeId, table.provider, table.sourceRef),
  }),
).enableRLS();
