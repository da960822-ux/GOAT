...
export type RecommendationWarningCode =
  ...
  | "TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE"
  | "COMPANION_RESERVED_FOR_LLM_COURSE";
...
export interface RecommendRequest extends ExposureStats, RouteDistanceInput {
  ...
  /** 동행 여부는 1차 카드 점수에는 직접 반영하지 않고, 선택 장소 이후 LLM 하루 코스 큐레이션에 사용한다. */
  companionType?: CompanionType | string;
  /** 기본값 false. 최종 MVP 흐름에서는 여행 목적을 1차 카드 점수에서 제외하고 2차 LLM 코스 선정에 사용한다. 과거 테스트/레거시 호환이 필요할 때만 true. */
  usePurposeInCardScore?: boolean;
...
export interface ConditionScoreBreakdown {
  ...
  purpose: {
    ...
    matched: boolean;
    score: number;
    usedInCardScore: boolean;
    usedInCoursePlanning: boolean;
    note?: string;
  };
  accessibility: {
    transportType?: string;
