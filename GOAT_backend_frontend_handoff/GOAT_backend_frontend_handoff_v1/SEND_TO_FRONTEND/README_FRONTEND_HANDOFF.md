...
| `visitTime` | 제거 권장. 보내도 점수 계산에는 사용하지 않음 |
...
## 2026-07-02 추가: 다시 추천 버튼 요청값

다시 추천 버튼을 누를 때는 직전 추천 결과의 `requestId`를 `rerollOfRequestId`로 보내면 된다.
...
const response = await fetch("/api/recommendations", {
  ...
  body: JSON.stringify({
    ...
    travelPurpose,
    transportType,
    rerollOfRequestId: previousRequestId,
  }),
});
```

백엔드는 해당 요청 ID로 이전 카드 3개의 `placeId`를 찾아 새 추천 후보에서 제외한다. 프론트는 별도로 이전 장소 ID를 계산할 필요 없이 `rerollOfRequestId`만 넘기면 된다.
