...
const TAG_ALIASES: Record<string, string[]> = {
  "바다": ["해변", "오션뷰", "해안", "수평선", "방파제", "해돋이", "에메랄드바다"],
  "해안도로": ["로드트립", "드라이브", "바다도로", "국도", "헌화로"],
  "초원": ["들판", "목장", "양떼목장", "하늘목장"],
  "고원": ["고랭지", "고랭지밭", "능선", "풍력발전기"],
  "겨울": ["설경", "설원", "눈꽃", "상고대"],
  "일본감성": ["일본", "일본풍", "일본소도시", "일본동네", "가마쿠라", "오노미치", "교토"],
  "유럽감성": ["유럽", "작은유럽", "유럽마을", "이탈리아", "영국", "프로방스"],
  "야간": ["저녁", "밤", "블루아워", "야간조명", "밤거리"],
  "레트로": ["빈티지", "복고", "브라운관TV"],
  "숲": ["숲길", "자작나무", "침엽수림", "흑림", "가문비나무"],
  "숙소": ["숙박", "료칸", "풀빌라", "자쿠지", "카라반", "산장", "체크인"],
  "협곡": ["주상절리", "잔도", "그랜드캐니언", "스카이워크", "출렁다리", "울렁다리"],
  "서핑": ["서프비치", "서프타운", "서핑숍", "서퍼", "비치사인"],
  "골목": ["골목산책", "소도시", "언덕길", "벽화마을"],
  "호수": ["반영", "데크길", "에메랄드호수"],
  "꽃밭": ["라벤더", "양귀비", "샤스타데이지", "꽃"],
  "폐광": ["산업유산", "수직갱", "재생관광"],
  "카페": ["디저트", "우드톤", "카페거리", "해변카페"]
};

const RELATED_TAGS: Record<string, string[]> = {
  "캘리포니아": ["해안도로", "로드트립", "서핑", "해변", "팜트리"],
  "발리": ["서핑", "해변", "비치클럽", "라탄", "프라이빗비치", "팜트리"],
  "산토리니": ["지중해", "흰건물", "종탑", "전망", "오션뷰", "유럽감성"],
  "알프스": ["목장", "초원", "고원", "설경", "유럽감성"],
  "북유럽": ["숲", "자작나무", "침엽수림", "겨울", "설경"],
  "일본소도시": ["가마쿠라", "철길", "골목", "서점", "카페"],
  "정원": ["유럽감성", "수목원", "꽃밭", "라벤더"],
  "시장": ["먹거리", "야간", "항구", "오사카", "후쿠오카"],
  "별": ["은하수", "밤", "새벽", "고원", "몽골"],
  "몽골": ["고원", "별", "은하수", "초원"],
  "아이슬란드": ["협곡", "주상절리", "에메랄드호수", "절벽"],
  "성당": ["유럽감성", "유럽마을", "고즈넉함"],
  "료칸": ["숙소", "자쿠지", "다도", "히노끼탕", "일본감성"],
  "카페거리": ["카페", "해변", "도시적"]
};

const MOOD_EXPANSION: Record<string, string[]> = {
  ...
  "조용함": ["숲", "정원", "서점", "성당", "골목", "호수"],
  "빈티지": ["레트로", "폐광", "시장", "브라운관TV", "구도심"]
};

const WEAK_STANDALONE_TAGS = new Set([
  "꽃밭", "폐광", "산", "호수", "발리", "산토리니", "이국적", "청량함", "낭만적", "탁트임", "도시적"
].map(normalizeTag));
...
function buildCandidatePool(input: RecommendationInput, places: GoatPlace[]) {
  ...
  const activePool = policy === "PRIMARY43"
    ? seedPool.filter((place) => place.dataStatus === "confirmed" && !isLodgingOrResort(place))
    : seedPool;
  return { seedPool, activePool, policy, reason };
}
...
function fallbackPlaces(input: RecommendationInput, activePool: GoatPlace[], seedPool: GoatPlace[]): ScoredPlace[] {
  ...
  return defaultIds
    .map((id, index) => {
      const place = lookupPool.find((item) => item.placeId === id) ?? activePool[index] ?? lookupPool[index];
    ...
    .filter((place): place is ScoredPlace => Boolean(place))
    .slice(0, 3);
}

export function getGoatRecommendations(input: RecommendationInput, places: GoatPlace[]): RecommendationApiResponse {
  try {
    const { seedPool, activePool, policy, reason } = buildCandidatePool(input, places);
    ...
    if (shouldRetryWithAll58(input, effectivePolicy, scored)) {
      const all58Scored = scoreCandidatePool(input, seedPool);
    ...
    const selected = fallbackUsed ? fallbackPlaces(input, effectiveActivePool, seedPool) : scored.slice(0, input.limit ?? 3);
