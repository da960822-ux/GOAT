import { CompanionType, GoatPlace, PrimaryTheme, PurposeTag, TransportType } from "./goatRecommendationTypes";

export type TourCandidateCategory = "TOUR" | "CAFE" | "RESTAURANT" | "MARKET" | "WALK" | "PHOTO" | "ETC";
export type CourseStopType = "START_PLACE" | "TOUR" | "CAFE" | "RESTAURANT" | "WALK" | "PHOTO" | "ETC";
...
export interface GoatDayCourseRequest extends CoursePlanningUserConditions {
  ...
  /** 한국관광콘텐츠랩 OpenAPI 호출 없이 테스트/프론트/백엔드에서 이미 확보한 주변 후보를 넘길 때 사용 */
  nearbyCandidates?: TourApiNearbyCandidate[];
  /** 후보가 너무 많을 때 LLM에 넘길 최대 개수. 기본 12개 */
  maxCandidatesForLlm?: number;
  /** true면 외부 API/LLM을 호출하지 않고 fallback 규칙으로만 코스를 만든다. 테스트와 발표 백업용 */
  forceRuleBasedFallback?: boolean;
  /** 한국관광콘텐츠랩 OpenAPI 위치기반 관광정보 조회 반경. 기본 3000m */
  radiusMeters?: number;
...
export interface GoatDayCourseResult {
  ...
  message: string;
  selectedPlace: Pick<GoatPlace, "place_id" | "place_name" | "city" | "region_group" | "primaryTheme" | "photo_point" | "place_type">;
