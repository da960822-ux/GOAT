import { recommendGoatPlaces, recommendGoatPlacesFromPhotoAnalysis, normalizePhotoAnalysisForRecommendation, createGoatRecommendation, createGoatDayCourse, InMemoryRecommendationExposureRepository, RecommendationExposureRepository, RecommendationExposureQuery, ExposureStats, SaveRecommendationExposuresInput } from "../src";
...
const logFilePath = path.resolve("tmp/travel-purpose-reserved-warning.jsonl");
  ...
  fallbackResult = recommendGoatPlaces(
    {
      ...
      logContext: { requestId: "test-travel-purpose-reserved" },
    },
    fallbackDataset,
    referenceDataset,
  );
...
assert(
  fallbackResult.resultData?.warnings.some((warning) => warning.code === "TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE"),
  "TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE warning should be emitted when travelPurpose is passed to the stage-1 card recommender",
);
assert(
  capturedServerLogs.some((args) => args[0] === "[GOAT_RECOMMENDATION_WARNING]" && String(JSON.stringify(args[1])).includes("TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE")),
  "TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE should be written to server console warning log automatically",
);
assert(fs.existsSync(logFilePath), "TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE should be written to warning log file automatically");
const logFileContent = fs.readFileSync(logFilePath, "utf8");
assert(logFileContent.includes("TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE"), "warning log file should contain TRAVEL_PURPOSE_RESERVED_FOR_LLM_COURSE");
...
assert(
  fallbackResult.resultData?.decisionAudit?.fallback.card3PurposeFallbackUsed === false,
  "final service flow should not use travelPurpose filtering/fallback in stage-1 card selection",
);
...
async function runServiceExposureTests(): Promise<void> {
  ...
  const courseResult = await createGoatDayCourse({
    selectedPlaceId: firstPlaceIds[0],
    primaryTheme: firstServiceResult.resultData?.request.primaryTheme ?? "바다·해안 무드",
    userMoodTags: firstServiceResult.resultData?.request.userMoodTags ?? [],
    userSceneTags: firstServiceResult.resultData?.request.userSceneTags ?? [],
    companionType: "친구",
    travelPurpose: "사진·포토스팟",
    transportType: "자차",
    forceRuleBasedFallback: true,
    nearbyCandidates: [
      { id: "near-001", title: "주변 감성 카페", category: "CAFE", distanceMeters: 500, source: "MOCK" },
      { id: "near-002", title: "주변 해안 산책로", category: "WALK", distanceMeters: 800, source: "MOCK" },
      { id: "near-003", title: "주변 로컬 맛집", category: "RESTAURANT", distanceMeters: 1000, source: "MOCK" },
    ],
  }, placesDataset);

  assert(courseResult.status === "DONE", "courseResult.status should be DONE");
  assert(courseResult.resultType === "COURSE", "courseResult.resultType should be COURSE");
  assert(courseResult.stops.length >= 2, "courseResult should contain selected place and at least one nearby stop");
  assert(courseResult.stops[0].id === firstPlaceIds[0], "course first stop should be the selected recommendation card place");
  assert(courseResult.conditions.companionType === "친구", "course flow should receive companionType for LLM/fallback planning");
  assert(courseResult.conditions.travelPurpose === "사진·포토스팟", "course flow should receive travelPurpose for LLM/fallback planning");
