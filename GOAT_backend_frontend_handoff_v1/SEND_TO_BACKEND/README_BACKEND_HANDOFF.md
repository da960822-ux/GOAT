...
| `data/goat_simplified_scoring_tags_v10_accessibility_merged.json` | 필수 | 61개 장소 데이터 + 태그 기준 |
...
| `excludeIds` | 선택 | `["GOAT-024"]` | 다시 추천 시 이미 노출한 장소 제외 |
...
현재 장소 JSON은 61개 장소를 포함하며, `address`, `latitude`, `longitude`는 일부 추가 확인 장소에서 비어 있을 수 있습니다. `imageUrl`도 비어 있을 수 있으므로 프론트에서 placeholder를 준비하면 됩니다.
