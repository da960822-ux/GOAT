...
| `best_time` | 추천 방문 시간대 | 사용자 방문 시간과 비교 | 카드/상세에 표시 |
...
| `address` | 상세 주소 | 지도앱 검색, 상세 정보 |
| `latitude` | 위도 | 거리 보정, 지도앱 연결 |
| `longitude` | 경도 | 거리 보정, 지도앱 연결 |
| `coordinateSource` | 좌표 출처 | 데이터 검증/운영 참고 |
...
| `address` | 채워짐 | 운영 중 검증 | 상세 주소, 지도앱 연결 |
| `latitude` | 채워짐 | 운영 중 검증 | 위도 |
| `longitude` | 채워짐 | 운영 중 검증 | 경도 |
| `ktoContentId` | 아직 없음 | KTO 상세 연동 시 | 이미지·설명·방문자 통계 연결 키 |
