...
function getTimeoutMs(): number {
  const configured = Number.parseInt(process.env.KAKAO_API_TIMEOUT_MS ?? "", 10);
  return Number.isFinite(configured)
    ? Math.max(10, Math.min(configured, DEFAULT_TIMEOUT_MS))
    : DEFAULT_TIMEOUT_MS;
}
...
async function fetchJson<T>(url: string, key: string | undefined, timeoutMs = getTimeoutMs()): Promise<T> {
