...
- 화면 스타일은 NativeWind v4와 Tailwind CSS v3 토큰을 기본으로 사용한다.
- 재사용 가능한 원시 UI는 React Native Reusables 방식으로 앱 소스에 소유한다. GOAT 전용 사진 카드·브랜드 락업·고정 CTA는 시안 전용 컴포넌트로 유지한다.
- Expo SDK 54의 Reanimated 4.1을 유지하며, 모션은 선택·저장·분석 상태 전달에만 사용한다. reduced-motion에서는 즉시 상태로 전환한다.
...
- 홈: 중앙 GOAT 락업, 해안 사진 히어로, 감성/사진 이중 CTA, 최근 추천·저장 카드, 하단 탭을 유지한다.
- 선택·조건: 사진 카드의 선택 체크와 네 조건군을 모두 보이고, 고정 CTA가 스크롤 콘텐츠를 가리지 않게 한다.
- 결과·상세: 장소 사진, 매치율, 태그, 사진 포인트, 저장·공유·지도·카카오 행동을 시안 순서로 배치한다.
- 로그인·분석·오류·결과 없음: 지도/산/해안 브랜드 아트, 중앙 락업, 한 개의 주요 행동과 명확한 복구 행동을 사용한다.
- 시안에 없는 보조 화면도 동일한 Forest/Ivory, Noto Serif KR/Pretendard, 얇은 구분선, 사진 중심 규칙을 적용한다.

## 자산 규칙

- 실제 장소 정보에는 검증된 장소 사진과 데이터만 사용한다. 생성 이미지는 지도 콜라주, 상태 배경, 업로드 배경 등 비사실성 브랜드 아트에 한정한다.
- 사진 프레임은 히어로 16:10, 선택 카드 16:7, 결과 카드 16:6을 기본으로 하고, 공통 어두운 오버레이와 초점 위치를 사용한다.
