...
function parseDeploymentTarget(rawValue, defaultProtocol = "https:") {
  const value = rawValue.trim();
  const parsed = new URL(
    /^https?:\/\//i.test(value) ? value : `${defaultProtocol}//${value}`,
  );
  return {
    host: parsed.host,
...
function getDeploymentTarget() {
  ...
  if (configured?.trim()) {
    return parseDeploymentTarget(configured);
  }

  if (process.argv.includes("--deployment")) {
    throw new Error(
      "Deployment builds require REPLIT_INTERNAL_APP_DOMAIN, REPLIT_DEV_DOMAIN, or EXPO_PUBLIC_DOMAIN.",
    );
  }

  const localPort = parseInt(process.env.PORT || "3000", 10);
  const safePort = Number.isInteger(localPort) && localPort > 0 && localPort <= 65535
    ? localPort
    : 3000;
  const localTarget = parseDeploymentTarget(`127.0.0.1:${safePort}`, "http:");
  console.log(`No deployment domain set; building for local serve at ${localTarget.baseUrl}`);
  return localTarget;
}
...
async function startMetro(expoPublicDomain, expoPublicReplId, apiBaseUrl) {
  ...
  const env = {
    ...
    // Production bundle generation is local and must not depend on Expo's
    // optional online dependency validation service being reachable.
  ...
  let expoCliPath;
  try {
    // Resolve from this workspace package instead of relying on a global pnpm shim.
    // Spawning Node with the resolved CLI also avoids cmd.exe quoting failures when
    // the Windows workspace path contains spaces or non-ASCII characters.
    expoCliPath = require.resolve("expo/bin/cli", { paths: [projectRoot] });
  } catch {
    throw new Error("Local Expo CLI was not found. Install workspace dependencies first.");
  }
  const spawnCommand = process.execPath;
  const spawnArgs = [expoCliPath, ...expoArgs];
  metroProcess = spawn(
    spawnCommand,
    spawnArgs,
    {
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      // Metro may serialize non-ASCII asset names as JSON unicode escapes.
      // Decode those escapes before resolving the real file on disk.
      const assetName = JSON.parse(`"${match[3]}"`);
      const assetType = JSON.parse(`"${match[4]}"`);
...
function rewriteLocalManifestAssetUrls(value, timestamp, baseUrl) {
  if (Array.isArray(value)) {
    return value.map((item) =>
      rewriteLocalManifestAssetUrls(item, timestamp, baseUrl),
    );
  }

  if (value && typeof value === "object") {
    for (const [key, item] of Object.entries(value)) {
      value[key] = rewriteLocalManifestAssetUrls(item, timestamp, baseUrl);
    }
    return value;
  }
  ...
  const relativeSource = decodeURIComponent(parsed.pathname.slice("/assets/".length))
    .replace(/^(\.\/)+/, "")
    .replace(/\\/g, "/");
  ...
  if (!sourcePath.startsWith(projectPrefix) || !fs.existsSync(sourcePath)) {
    throw new Error(`Manifest asset not found inside project: ${relativeSource}`);
  }
  ...
  const outputPath = path.join(projectRoot, "static-build", ...publicPath.split("/"));
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.copyFileSync(sourcePath, outputPath);

  return `${baseUrl}${basePath}/${publicPath}`;
}

function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const deploymentHost = new URL(baseUrl).host;
  const updateForPlatform = (platform, manifest) => {
    ...
    manifest.extra.expoClient.hostUri = `${deploymentHost}/${platform}`;
    manifest.extra.expoGo.debuggerHost = `${deploymentHost}/${platform}`;
    manifest.extra.expoGo.packagerOpts.dev = false;
    rewriteLocalManifestAssetUrls(manifest, timestamp, baseUrl);
...
async function main() {
  ...
  const baseUrl = deploymentTarget.baseUrl;
  const timestamp = `${Date.now()}-${process.pid}`;

  prepareDirectories(timestamp);
  clearMetroCache();

  await startMetro(deploymentTarget.host, expoPublicReplId, baseUrl);
