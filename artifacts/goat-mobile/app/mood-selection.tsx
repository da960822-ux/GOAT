...
export default function MoodSelectionScreen() {
  ...
  return <View style={[styles.screen, { paddingTop: insets.top }]}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: 100 + insets.bottom }]}>
      ...
      <View accessibilityRole="radiogroup" style={styles.cards}>{moods.map((item, moodIndex) => {
        const active = item.id === selected;
        const featured = moodIndex === 0;
        return <Pressable key={item.id} accessibilityRole="radio" accessibilityState={{ checked: active }} accessibilityLabel={`${item.name}. ${item.description}. 사진은 ${item.cover.picturedPlaceName}. ${item.cover.attribution}`} onPress={() => setSelected(item.id)} style={({ pressed }) => [styles.card, featured && styles.featuredCard, active && styles.cardActive, pressed && styles.pressed]}>
          ...
          <LinearGradient colors={["rgba(8,25,21,.06)", "rgba(8,25,21,.82)"]} locations={[.25, 1]} style={[styles.scrim, active && styles.scrimActive]} pointerEvents="none" />
          <View style={[styles.cardCopy, featured && styles.featuredCardCopy]}><Text style={[styles.cardName, featured && styles.featuredCardName]}>{item.name}</Text><Text style={styles.cardDescription}>{item.description}</Text><Text numberOfLines={1} style={styles.cardPlace}>영감 장소 · {item.cover.picturedPlaceName}</Text></View>
...
const styles = StyleSheet.create({
  ...
  cards: { marginTop: 24, gap: 13, flexDirection: "row", flexWrap: "wrap" }, card: { width: "48.2%", minHeight: 184, borderRadius: radius.md, overflow: "hidden", borderWidth: 2, borderColor: "transparent", justifyContent: "flex-end" }, featuredCard: { width: "100%", minHeight: 292, borderRadius: 22 }, cardActive: { borderColor: palette.forest }, scrim: { ...StyleSheet.absoluteFillObject }, scrimActive: { backgroundColor: "rgba(8,25,21,.2)" }, cardCopy: { minHeight: 184, justifyContent: "flex-end", padding: 15 }, featuredCardCopy: { minHeight: 292, padding: 20, paddingTop: 96 }, cardName: { paddingRight: 54, fontFamily: fonts.serif, fontSize: 20, lineHeight: 28, letterSpacing: -0.7, color: palette.white }, featuredCardName: { fontSize: 29, lineHeight: 38 }, cardDescription: { marginTop: 4, paddingRight: 44, fontFamily: fonts.body, fontSize: 13, lineHeight: 19, color: palette.white }, cardPlace: { marginTop: 7, paddingRight: 42, fontFamily: fonts.semibold, fontSize: 12, lineHeight: 17, color: palette.white }, select: { position: "absolute", top: 15, right: 15, width: 48, height: 48, borderRadius: 24, alignItems: "center", justifyContent: "center", backgroundColor: palette.paper }, selectActive: { backgroundColor: palette.forest },
