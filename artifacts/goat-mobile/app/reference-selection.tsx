...
export default function ReferenceSelectionScreen() {
  ...
  return <View style={[styles.screen, { paddingTop: insets.top }]}>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.content, { paddingBottom: 110 + insets.bottom }]}>
      ...
      <View accessibilityRole="radiogroup" style={styles.grid}>{cards.map((card, cardIndex) => {
        const referenceCardId = card.id;
        const active = selected === referenceCardId;
        const featured = cardIndex === 0 && !filter;
        return <Pressable key={referenceCardId} accessibilityRole="radio" accessibilityState={{ checked: active }} accessibilityLabel={`${card.name}. ${card.category}`} onPress={() => setSelected(active ? null : referenceCardId)} style={({ pressed }) => [styles.card, featured && styles.featuredCard, active && styles.cardActive, pressed && styles.pressed]}>
          ...
          <View style={[styles.cardCopy, featured && styles.featuredCardCopy]}><Text numberOfLines={1} style={[styles.cardTitle, featured && styles.featuredCardTitle]}>{card.name}</Text><Text numberOfLines={1} style={styles.cardSubtitle}>{card.region} · {card.category}</Text></View>
        </Pressable>;
      })}</View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  grid: { flexDirection: "row", flexWrap: "wrap", gap: 12 }, card: { width: "48.2%", minHeight: 188, overflow: "hidden", borderRadius: 18, borderWidth: 2, borderColor: "transparent", backgroundColor: palette.sage }, featuredCard: { width: "100%", minHeight: 256 }, cardActive: { borderColor: palette.forest }, scrim: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(11,34,27,.38)" }, scrimActive: { backgroundColor: "rgba(11,34,27,.56)" }, select: { position: "absolute", top: 10, right: 10, width: 36, height: 36, borderRadius: 18, borderWidth: 1, borderColor: "rgba(255,255,255,.8)", backgroundColor: "rgba(246,242,233,.78)", alignItems: "center", justifyContent: "center" }, selectActive: { backgroundColor: palette.forest, borderColor: palette.forest }, cardCopy: { minHeight: 188, justifyContent: "flex-end", padding: 13, paddingTop: 52 }, featuredCardCopy: { minHeight: 256, padding: 13, paddingTop: 52 }, cardTitle: { fontFamily: fonts.serif, fontSize: 17, lineHeight: 24, color: palette.white }, featuredCardTitle: { fontSize: 25, lineHeight: 32 }, cardSubtitle: { marginTop: 4, fontFamily: fonts.body, fontSize: 10.5, lineHeight: 15, color: "rgba(255,255,255,.9)" }, pressed: { opacity: 0.86 },
