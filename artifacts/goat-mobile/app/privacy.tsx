...
const SECTIONS = [
  {
    title: '수집하는 개인정보',
    body: 'GOAT 앱은 어떠한 개인정보도 수집하지 않습니다. 회원가입, 로그인 기능이 없으며, 앱 사용 중 입력되는 모든 선택 정보(감성 선택, 여행 조건 등)는 기기 내에서만 처리됩니다.',
  },
  {
    title: '위치정보 수집',
    body: '현재 위치는 출발지 기준 이동 정보와 추천 장소 간 연계 거리를 제공하기 위해 사용되며, 사용자가 직접 "현재 위치 사용"을 탭하여 권한을 허용한 경우에만 한 번 가져옵니다. 위치는 지속적으로 추적하지 않으며, 추천 흐름이 끝나면 기기 메모리에서 즉시 삭제됩니다.\n\n위치 권한을 허용하지 않아도 감성 기반 추천은 정상적으로 이용할 수 있습니다.\n\n지도 앱 연동 시 외부 앱(카카오맵)으로 이동하며, 해당 앱의 개인정보 처리방침이 적용됩니다.',
  },
  {
    title: '사진 및 이미지',
    body: '사진으로 감성 찾기 기능 사용 시 선택한 이미지는 감성 태그 추정 흐름에만 사용됩니다. 이미지는 서버로 전송되거나 영구 저장되지 않습니다.\n\n결과 화면과 장소 상세에 표시되는 사진은 한국관광공사 공공 API에서 제공하는 이미지이며, 사용자의 개인 이미지와 무관합니다.',
  },
  {
    title: '공공 API 연동',
    body: '앱은 한국관광공사(KTO) 공공데이터 API를 사용해 관광사진, 장소 정보, 방문 추이 예측 데이터를 조회합니다. 이 과정에서 장소명 등 검색 키워드가 API에 전송되지만, 사용자를 특정할 수 있는 정보는 포함되지 않습니다. API 키는 앱 외부에 노출되지 않습니다.',
  },
  {
    title: '분석 및 추적',
  ...
  {
    title: '제3자 서비스',
    body: '지도 버튼 클릭 시 카카오맵 앱이 실행됩니다. 해당 서비스의 개인정보 처리방침은 카카오 정책을 따릅니다. 한국관광공사 공공 API는 공공데이터포털(data.go.kr) 이용 약관을 따릅니다.',
  },
  {
    title: '문의',
