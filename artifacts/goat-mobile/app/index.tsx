    ...
    {state === "loading" ? <ScreenState title="장면을 불러오는 중이에요"><ActivityIndicator color={palette.forest} /></ScreenState>
      ...
      : <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            return <View><SceneCoverCard number={index + 1} featured={index === 0} title={item.title} description={item.availability === "AVAILABLE" ? item.description : `${item.description} · 준비 중`} picturedPlaceName={picturedPlaceName} imageUri={photo?.url} imageSource={editorial?.source} imageCachePolicy={photo ? getPhotoCachePolicy(photo) : editorial ? "memory-disk" : "none"} attribution={attribution} selected={busyId === item.selectionId} disabled={item.availability !== "AVAILABLE" || busyId !== null} onPress={() => void choose(item)} />{busyId === item.selectionId ? <View style={styles.busy} accessibilityLiveRegion="polite"><ActivityIndicator size="small" color={palette.forest} /><Text style={styles.busyText}>어울리는 세 곳을 찾고 있어요</Text></View> : null}</View>;
          }}
