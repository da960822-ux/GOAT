...
} from '@/src/components/RecommendationRoleBadge';
...
function normalizeCoursePurpose(
  purpose?: string | null
) {
  if (!purpose) {
    return undefined;
  ...
  if (
    purpose.includes('액티비티') ||
    purpose.includes('체험')
  ) {
    return '체험·액티비티';
  }

  if (purpose.includes('카페')) {
    return '카페·실내휴식';
  }
...
function normalizeTransport(
  ...
  if (transport.includes('대중')) {
    return '대중교통';
  }

  if (
    transport.includes('도보') ||
    transport.includes('뚜벅')
  ) {
    return '도보중심';
  }
...
export default function DetailScreen() {
  ...
  async function loadCourseForPlace(
    targetPlace: Place
  ) {
    ...
    try {
      const result =
        await recommendCourse({
          selectedPlaceId:
            targetPlace.place_id,

          primaryTheme:
            targetPlace.primary_mood as any,

          userMoodTags:
            targetPlace.mood_tags.slice(
              0,
              5
            ),
          ...
          companionType:
            travelPreferences?.companion as any,

          travelPurpose:
            normalizeCoursePurpose(
      ...
      setCourseError(
        '하루 코스를 불러오지 못했어요. 잠시 후 다시 시도해 주세요.'
      );
    } finally {
  ...
  async function handleShare() {
    ...
    try {
      await Share.share({
        title:
          `${place.place_name} · GOAT`,

        message: [
          `GOAT 추천 장소: ${place.place_name}`,
  ...
  async function handleKakaoMap() {
    ...
    const coords =
      ...
      tourInfo?.longitude
        ? {
            lat: tourInfo.latitude,
            lng: tourInfo.longitude,
          }
        : undefined;
      ...
      >
        {hasPhoto ? (
          ...
          >
            ...
            <View
              style={
                styles.photoHeroScrim
              }
            />

            <View
              style={
                styles.photoHeroOverlay
              }
            >
              <View
                style={
                  styles.typeBadgeDark
                }
              >
                <Text
                  style={
                    styles.typeBadgeDarkText
                  }
                >
                  {place.place_type}
                </Text>
...
function KakaoCourseMap({
  ...
  course: RecommendCourseData;
  colors: any;
}) {
  const config =
    course.staticMap
      .staticMapConfig as any;
  ...
  useEffect(() => {
    ...
    const rawSdkScriptUrl =
      String(
        config.sdkScriptUrl
      );

    const sdkScriptUrl =
      rawSdkScriptUrl.includes(
        'autoload='
      )
        ? rawSdkScriptUrl
        : `${
...
function CourseRecommendationSection({
  ...
  const mapUrl =
    course?.staticMap
      .fallbackMapSearchUrl;

  return (
    <InfoCard
      colors={colors}
      icon="map"
      title="선택 장소 중심 하루 코스"
    >
      {loading && (
        <View
          style={
            styles.courseLoadingRow
          }
        >
          <ActivityIndicator
            size="small"
              ...
              {course.stops.map(
                (
                  ...
                  <View
                    ...
                    style={
                      styles.courseStopRow
                    }
                  >
                    <View
                      style={
                        styles.courseTimeline
                      }
                    >
                      <View
                        style={[
                          styles.courseOrderDot,
                          {
                            backgroundColor:
                              accentColor,
                          },
                    ...
                    <View
                      style={
                        styles.courseStopContent
                      }
                    >
                      <View
                        style={
                          styles.courseStopHeader
                        }
                      >
                        <Text
                          style={[
                            styles.courseStopTitle,
                            {
                              color:
                                colors.foreground,
                            },
