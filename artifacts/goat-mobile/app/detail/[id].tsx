...
import { AccessibilityInfo, ActivityIndicator, Alert, findNodeHandle, InteractionManager, Modal, Platform, Pressable, ScrollView, Share, StyleSheet, Text, useWindowDimensions, View } from "react-native";
...
export default function DetailScreen() {
  ...
  return <View style={styles.screen}>
    ...
    <Gallery visible={galleryVisible} name={place.place_name} photos={photos} returnFocusRef={galleryTriggerRef} onClose={() => setGalleryVisible(false)} />
...
function Gallery({ visible, name, photos, returnFocusRef, onClose }: { visible: boolean; name: string; photos: PlacePhotosData | null; returnFocusRef: React.RefObject<View | null>; onClose: () => void }) {
  ...
  const maxFrameHeight = Math.max(160, height - Math.max(insets.top, 12) - Math.max(insets.bottom, 12) - 150);
  const galleryPosition = images.length ? `${current + 1} / ${images.length}` : "사진 없음";
  useEffect(() => { if (visible) setCurrent(0); }, [visible]);
  ...
  const handleShow = () => { wasVisible.current = true; setCurrent(0); pagerRef.current?.scrollTo({ x: 0, animated: false }); focus(closeRef.current); };
  ...
  return <Modal visible={visible} animationType="fade" onRequestClose={handleClose} onShow={handleShow}><View accessibilityViewIsModal style={styles.gallery}><View style={[styles.galleryHead, { paddingTop: Math.max(insets.top, 12) }]}><View style={styles.galleryHeading}><Text lineBreakStrategyIOS="hangul-word" textBreakStrategy="balanced" android_hyphenationFrequency="none" style={styles.galleryTitle}>{name}</Text><Text accessibilityLiveRegion="polite" style={styles.galleryPosition}>{galleryPosition}</Text></View><Pressable ref={closeRef} accessibilityRole="button" accessibilityLabel="사진 닫기" onPress={handleClose} style={styles.galleryClose}><BrandIcon name="close" color={palette.white} /></Pressable></View>{images.length ? <ScrollView ref={pagerRef} style={styles.galleryPager} horizontal pagingEnabled decelerationRate="fast" showsHorizontalScrollIndicator={false} onMomentumScrollEnd={(event) => setCurrent(Math.round(event.nativeEvent.contentOffset.x / width))}>{images.map((image, index) => { const credit = [image.attribution.label, image.attribution.author].filter(Boolean).join(" "); const ratio = imageRatios[image.url] ?? 4 / 3; const resolvedFrameWidth = Math.min(frameWidth, maxFrameHeight * ratio); return <View key={image.url} style={[styles.galleryPage, { width }]}><View style={[styles.galleryFrame, { width: resolvedFrameWidth, aspectRatio: ratio }]}><Image source={{ uri: image.url }} style={StyleSheet.absoluteFillObject} contentFit="contain" accessibilityLabel={`${name} 실제 사진 ${index + 1}`} onLoad={({ source }) => { if (source.width > 0 && source.height > 0) setImageRatios((ratios) => ratios[image.url] ? ratios : { ...ratios, [image.url]: source.width / source.height }); }} /></View>{credit ? <Text style={styles.galleryCredit}>사진 출처 · {credit}</Text> : null}</View>; })}</ScrollView> : <View style={styles.galleryEmpty}><Text style={styles.galleryEmptyText}>{photos?.galleryStatus === "ERROR" ? "사진을 불러오지 못했어요." : "확보된 추가 사진이 없어요."}</Text></View>}<View style={{ height: Math.max(insets.bottom, 12) }} /></View></Modal>;
}
...
const styles = StyleSheet.create({
  ...
  gallery: { flex: 1, backgroundColor: palette.forestDeep }, galleryHead: { minHeight: 82, paddingHorizontal: 18, paddingBottom: 10, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }, galleryHeading: { flex: 1, gap: 3 }, galleryTitle: { fontFamily: fonts.serif, fontSize: 20, lineHeight: 28, color: palette.white }, galleryPosition: { fontFamily: fonts.medium, fontSize: 12, color: palette.sage, fontVariant: ["tabular-nums"] }, galleryClose: { width: 48, height: 48, alignItems: "center", justifyContent: "center" }, galleryPager: { flex: 1 }, galleryPage: { alignItems: "center", justifyContent: "flex-start", paddingTop: 12, paddingHorizontal: 20 }, galleryFrame: { overflow: "hidden", borderRadius: radius.md, backgroundColor: palette.forest }, galleryCredit: { width: "100%", marginTop: 12, fontFamily: fonts.body, fontSize: 12, lineHeight: 18, color: palette.white }, galleryEmpty: { flex: 1, alignItems: "center", justifyContent: "center", padding: 24 }, galleryEmptyText: { fontFamily: fonts.body, fontSize: 15, color: palette.white },
