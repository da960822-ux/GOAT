...
export default function ResultsScreen() {
  ...
  if (recommendations.length === 0) {
    return (
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        <Header title="추천 결과" onBack={() => router.back()} />
        <EmptyState
          title="조건에 맞는 추천을 찾지 못했어요"
          description="조건을 조금 넓히거나 다른 감성을 선택해 주세요."
          actionLabel="여행 조건 다시 고르기"
          onAction={() => router.replace('/travel-preference')}
        />
      </View>
    );
  }
  ...
  async function handleReRecommend() {
    if (!selectedMood || !travelPreferences || isRecommending || recommendInFlight.current) return;
    ...
    try {
      const result = await recommendFromTags(buildRecommendationRequest({
        ...
        sessionId,
        rerollOfRequestId: lastRecommendationRequestId,
      }));
  ...
  const conditionParts = [
    ...
    travelPreferences?.season ?? null,
    travelPreferences?.purpose,
  ].filter(Boolean) as string[];

  const showLocationChip = origin?.type === 'current' || origin?.type === 'region' || origin?.type === 'address';
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.75}
          accessibilityRole="button"
          accessibilityState={{ disabled: isRecommending, busy: isRecommending }}
          testID="recommend-again"
        >
