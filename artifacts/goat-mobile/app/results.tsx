...
import {
  ...
  diffPlaceIds,
  discardConditionUndo,
  resetConditionDraft,
  restoreUndo,
  syncConditionResult,
  updateConditionDraft,
  type ConditionState,
} from "@/src/components/discovery/conditionState";
...
import {
  hydrateDisplayCards,
  replacePublicCard,
  requestPublicRecommendation,
  StaleDiscoveryResponse,
  type DisplayCard,
} from "@/src/services/publicDiscovery";
...
export default function ResultsScreen() {
  ...
  const [seenIds, setSeenIds] = useState(
    () => publicRecommendation?.cards.map((card) => card.placeId) ?? [],
  );
  const [conditionState, setConditionState] = useState<ConditionState<
    DiscoveryConditions,
    PublicRecommendationData
  > | null>(null);
  ...
  useEffect(() => {
    if (!publicRecommendation) return;
    let active = true;
    setCards(
      publicRecommendation.cards.map((card) => ({ ...card, place: null })),
    );
    hydrateDisplayCards(publicRecommendation.cards).then((next) => {
  ...
  useEffect(() => {
    if (publicRecommendation) return;
    let active = true;
    const restart = async (message?: string) => {
      await localSceneStore.clearDraft().catch(() => undefined);
      if (!active) return;
      if (message) Alert.alert("이전 추천을 새로 시작할게요", message);
      router.replace("/");
    };
    void (async () => {
      try {
        const [loaded, selectionsResponse] = await Promise.all([
          localSceneStore.loadDraft(),
          getPublicSelections(),
        ]);
        if (loaded.status === "EMPTY") return void restart();
        if (loaded.status === "STALE")
          return void restart("저장된 추천 형식을 확인할 수 없어요.");
        const selection = selectionsResponse.data.selections.find(
          ({ selectionId }) => selectionId === loaded.draft.selectionId,
        );
        if (!selection)
          return void restart("저장한 장면이 현재 목록에 없어요.");
        const [next, placeChecks] = await Promise.all([
          requestPublicRecommendation({
            selectionId: loaded.draft.selectionId,
            mode: loaded.draft.mode,
            restoreDraft: {
              placeIds: loaded.draft.placeIds,
              seenIds: loaded.draft.seenIds,
              revision: loaded.draft.revision,
            },
          }),
          Promise.allSettled(
            loaded.draft.placeIds.map((placeId) => getPlace(placeId)),
          ),
        ]);
        const validPlaceIds = loaded.draft.placeIds.filter(
          (_, placeIndex) => placeChecks[placeIndex]?.status === "fulfilled",
        );
        const restored = await localSceneStore.restoreDraft({
          catalogVersion: next.catalogVersion,
          policyVersion: next.policyVersion,
          selectionIds: selectionsResponse.data.selections.map(
            ({ selectionId }) => selectionId,
          ),
          placeIds: validPlaceIds,
        });
        if (restored.status !== "RESTORED")
          return void restart("저장된 추천이 현재 카탈로그와 맞지 않아요.");
        if (!active) return;
        setSeenIds([
          ...new Set([
            ...loaded.draft.seenIds,
        ...
        if (active) setRestoreError("이전 추천을 불러오지 못했어요.");
      }
    })();
    return () => {
      active = false;
  ...
  const todayCopy = useMemo(() => {
    if (
      !publicRecommendation ||
      publicRecommendation.todayStatus === "NOT_REQUESTED"
    )
      return "오늘 조건 반영하기";
  ...
  if (!publicRecommendation)
    return (
      <View style={styles.restoreState}>
        <ActivityIndicator color={palette.forest} />
        <Text style={styles.restoreText}>
          {restoreError ?? "이전 추천을 불러오는 중이에요"}
        </Text>
        {restoreError ? (
          <Pressable
          ...
          >
            <Text style={styles.restoreButtonText}>새로 시작하기</Text>
          </Pressable>
        ) : null}
      </View>
    );

  const conditions = conditionState?.committed.conditions ?? {
    transport: undefined,
    today: publicRecommendation.mode === "TODAY",
  };

  const applyRecommendation = (
    next: PublicRecommendationData,
    message: string,
    persist = true,
    discardUndo = false,
  ) => {
    ...
    setConditionState((current) =>
      current
        ? discardUndo
          ? discardConditionUndo(syncConditionResult(current, next))
          : syncConditionResult(current, next)
        : current,
    );
    setSeenIds((previous) => [
      ...new Set([...previous, ...next.cards.map((card) => card.placeId)]),
    ]);
    ...
    if (persist)
      void localSceneStore
        .saveDraft({
          ...
          placeIds: next.cards.map((card) => card.placeId),
          seenIds: [
  ...
  const applyConditions = async () => {
    ...
    try {
      const next = await requestPublicRecommendation({
        selectionId: publicRecommendation.selectionId,
        mode: draft.today ? "TODAY" : "SCENE",
        transportType: draft.transport,
      });
      if (generation !== requestGeneration.current) return;
      const change = diffPlaceIds(
        conditionState.committed.result.cards.map((card) => card.placeId),
        next.cards.map((card) => card.placeId),
      );
      setConditionState((current) =>
      ...
      if (
        ...
        !(error instanceof StaleDiscoveryResponse)
      )
        setConditionError(
          "조건을 반영하지 못했어요. 기존 세 곳은 그대로 유지했어요.",
        );
    } finally {
  ...
  const closeConditions = () => {
    ...
    setConditionError(null);
    setConditionState((current) =>
      current ? resetConditionDraft(current) : current,
    );
    setConditionVisible(false);
  };
  ...
  const replace = async (card: DisplayCard, slot: number) => {
    ...
    try {
      const next = await replacePublicCard(
        publicRecommendation,
        slot,
        seenIds,
        card.replaceOptions[0] ?? "ANY",
  ...
  const openDecision = (card: DisplayCard) => {
    setSaveStatus("idle");
    setShareUrl(null);
    setSaveNote("");
    ...
    void localSceneStore
      ...
      .then((scenes) =>
        setSaveNote(
          scenes.find(({ placeId }) => placeId === card.placeId)?.note ?? "",
        ),
      )
      .catch(() => undefined);
  };
  ...
  const openMap = () => {
    if (selectedCard?.place)
      void openKakaoMap(
        selectedCard.place,
        selectedCard.place.lat != null && selectedCard.place.lng != null
          ? { lat: selectedCard.place.lat, lng: selectedCard.place.lng }
          : undefined,
      );
    else Alert.alert("지도 정보를 불러오지 못했어요");
  };
  const continueCourse = async () => {
    if (!selectedCard?.place || courseStatus === "loading") return;
    setCourseStatus("loading");
    try {
      const next = await createCourse(
        selectedCard.place,
        selectedCard.matchedFeatures,
      );
      setCourse(next);
  ...
  const share = async () => {
    if (!selectedCard?.place || !API_BASE_URL) {
      Alert.alert(
        "공개 링크를 만들 수 없어요",
        "운영 웹 주소 연결이 필요해요.",
      );
    ...
    const payload = buildPublicPlaceShare(
      ...
      selectedCard.place.place_name,
      publicRecommendation.selectionId,
    );
    try {
      await Share.share({
        title: payload.title,
  ...
  const goTo = (next: number) => {
    const safe = Math.max(0, Math.min(cards.length - 1, next));
    setIndex(safe);
    deckRef.current?.scrollTo({ x: safe * (cardWidth + gap), animated: true });
    AccessibilityInfo.announceForAccessibility(
      `카드 ${safe + 1}/${cards.length}`,
    );
  };
  ...
  return (
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="장면 선택으로 돌아가기"
          onPress={() => router.replace("/")}
          style={styles.iconButton}
        >
          <BrandIcon name="back" color={palette.forest} />
        </Pressable>
        <Text style={styles.headerTitle}>추천한 세 곳</Text>
      ...
      >
        ...
        >
          {cards.map((card, cardIndex) => (
            <View key={card.placeId} style={{ width: cardWidth }}>
              <DecisionCard
                index={cardIndex + 1}
                total={cards.length}
                ...
                replacementState={
                  card.canReplace && card.replacementCount > 0
                    ? replacingId === card.placeId
                      ? "loading"
                      : "available"
                    : "unavailable"
                }
                replacementDisabled={conditionLoading || replacingId !== null}
                onDetails={() =>
                  router.push({
                    pathname: "/detail/[id]",
                    params: {
                ...
                onReplace={() => void replace(card, cardIndex + 1)}
                onChoose={(trigger) => {
                  decisionTriggerRef.current = trigger ?? null;
                  openDecision(card);
                }}
              />
            </View>
        ...
        <View style={styles.deckNav}>
          ...
          >
            <BrandIcon name="back" size={18} color={palette.forest} />
            <Text style={styles.navText}>이전</Text>
          </Pressable>
          <Text style={styles.nextHint}>
            {index < cards.length - 1
              ? `다음은 ${cards[index + 1]?.place?.place_name ?? "또 다른 장소"}`
              : "세 곳을 모두 봤어요"}
          </Text>
          <Pressable
            accessibilityRole="button"
            ...
            style={[
              styles.navButton,
              index >= cards.length - 1 && styles.navDisabled,
            ]}
          >
            <Text style={styles.navText}>다음</Text>
            <BrandIcon name="arrow-right" size={18} color={palette.forest} />
          </Pressable>
        </View>
        <View style={styles.controls}>
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel="추천 조건 조정"
            accessibilityState={{
              disabled: conditionLoading || replacingId !== null,
            }}
            disabled={conditionLoading || replacingId !== null}
            onPress={() => {
              setConditionError(null);
            ...
            style={[
              ...
              (conditionLoading || replacingId !== null) &&
                styles.controlDisabled,
            ]}
          >
            <BrandIcon name="transport" size={18} color={palette.forest} />
            <View style={styles.conditionCopy}>
              <Text style={styles.conditionTitle}>추천 조건 조정</Text>
              <Text
                style={styles.conditionSummary}
              >{`${conditions.transport === "CAR" ? "자차 이동" : conditions.transport === "PUBLIC_TRANSIT" ? "대중교통 이동" : "분위기 우선"} · ${conditions.today ? todayCopy : "오늘 조건 미반영"}`}</Text>
            </View>
            <BrandIcon name="arrow-right" size={18} color={palette.forest} />
          </Pressable>
          {conditionState?.undo ? (
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="이전 세 곳으로 되돌리기"
              accessibilityState={{
                disabled: conditionLoading || replacingId !== null,
              }}
              disabled={conditionLoading || replacingId !== null}
              onPress={undoConditions}
              style={[
                styles.undoButton,
                (conditionLoading || replacingId !== null) &&
                  styles.controlDisabled,
            ...
            >
              <BrandIcon name="refresh" size={17} color={palette.forest} />
              <Text style={styles.undoText}>이전 결과로 되돌리기</Text>
            </Pressable>
          ) : null}
          {factorCopy(publicRecommendation) ||
          publicRecommendation.partialApplied ? (
            <View>
              <Pressable
                accessibilityRole="button"
              ...
              {diagnosticsVisible ? (
                <View style={styles.diagnostics}>
                  ...
                  {publicRecommendation.partialApplied ? (
                    <Text style={styles.partial}>일부 정보만 반영했어요</Text>
                  ) : null}
                </View>
              ) : null}
            </View>
        ...
        <Pressable
          ...
          accessibilityRole="button"
          accessibilityLabel="세 곳 비교 열기"
          onPress={() => setCompareVisible(true)}
          style={styles.compareButton}
        >
          <Text style={styles.compareText}>세 곳 한눈에 보기</Text>
          <BrandIcon name="arrow-right" size={18} color={palette.forest} />
        </Pressable>
      </ScrollView>
      <CompareSheet
        ...
        onChoose={(card) => {
          ...
          decisionTriggerRef.current = compareTriggerRef.current;
          InteractionManager.runAfterInteractions(() => openDecision(card));
        }}
      />
      {conditionState ? (
        <ConditionSheet
          visible={conditionVisible}
          draft={conditionState.draft}
          loading={conditionLoading}
          disabled={replacingId !== null}
          error={conditionError}
          returnFocusRef={conditionTriggerRef}
          onChange={(draft) =>
            setConditionState((current) =>
              current ? updateConditionDraft(current, draft) : current,
            )
          }
          onApply={() => void applyConditions()}
      ...
      {selectedCard ? (
        <DecisionSheet
          ...
          selectedPlace={{
            region: selectedCard.place?.city ?? "강원",
            name: selectedCard.place?.place_name ?? "추천 장소",
          }}
          criticalRestriction={restrictionFor(selectedCard)}
          saveStatus={saveStatus}
          courseStatus={courseStatus}
          saveError="저장하지 못했어요. 이 화면에서 다시 시도해 주세요."
...
function CompareSheet({
  ...
  onClose: () => void;
  onChoose: (card: DisplayCard) => void;
}) {
  ...
  const handleChoose = (card: DisplayCard) => {
    wasVisible.current = false;
    onChoose(card);
  };
      ...
      <View style={styles.modalBackdrop}>
        <View
          ...
          style={[
            styles.compareSheet,
            {
              maxHeight: height - Math.max(insets.top, 12),
            ...
            {cards.map((card, index) => {
              ...
              return (
                <View key={card.placeId} style={styles.compareRow}>
                  <View style={styles.compareCardHead}>
                    {card.placeHero?.url ? (
                      <View>
                        ...
                        />
                        {photoSource ? (
                          <Text style={styles.comparePhotoSource}>
                            {photoSource}
                          </Text>
                        ) : null}
                      </View>
                    ...
                    <View style={styles.compareNameWrap}>
                  ...
                  <CompareAxis
                    label="사진에서 보이는 차이"
                    value={photoDifferenceFor(card)}
                  />
                  <CompareAxis
                    label="접근·이동"
                    value={
                      card.place?.accessibility?.trim() ||
                      "확인된 접근·이동 정보가 없어요."
                    }
                  />
                  <CompareAxis
...
function CompareAxis({
  ...
  return (
    <View style={styles.compareAxis}>
      <Text style={styles.compareAxisLabel}>{label}</Text>
      <Text
        style={[
          styles.compareAxisValue,
          critical && styles.compareAxisCritical,
        ]}
      >
        {value}
      </Text>
...
function photoDifferenceFor(card: DisplayCard) {
  return card.differenceNote?.trim() || "확인된 사진 차이 정보가 없어요.";
}

function summaryFor(card: DisplayCard) {
  return (
    card.differenceNote ??
...
function restrictionFor(card: DisplayCard) {
  const note = card.place?.note?.trim();
...
function factorCopy(data: PublicRecommendationData) {
  const labels = {
    WEATHER: "날씨",
    VISIT_CONCENTRATION: "방문 집중도",
  } as const;
  const reasons = {
    TIMEOUT: "정보 확인이 지연됐어요",
    NO_DATA: "확인 가능한 정보가 없어요",
    NOT_COMPARABLE: "비교할 정보가 충분하지 않아요",
    SINGLE_CANDIDATE: "비교할 후보가 더 필요해요",
    INVALID_DATA: "일부 정보를 확인할 수 없어요",
  } as const;
  const applied = data.appliedFactors
    .map((factor) => labels[factor])
    .join(" · ");
  const skipped = data.skippedFactors
    .map(({ factor, reason }) => `${labels[factor]}(${reasons[reason]})`)
    .join(" · ");
  return [
    applied ? `반영: ${applied}` : "",
    skipped ? `미반영: ${skipped}` : "",
...
const styles = StyleSheet.create({
  ...
  restoreText: { fontFamily: fonts.body, fontSize: 15, color: palette.muted },
  restoreButton: {
    minHeight: 48,
  ...
  modalBackdrop: {
    ...
    backgroundColor: "rgba(15,48,42,.52)",
  },
  compareSheet: {
    paddingHorizontal: 20,
    paddingTop: 12,
