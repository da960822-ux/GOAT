...
const inFlight = new Map<string, Promise<KTOTourInfo>>();
 ...
 */
async function fetchTourInfo(placeName: string, city: string): Promise<KTOTourInfo> {
...
export function getTourInfo(placeName: string, city: string): Promise<KTOTourInfo> {
  if (!placeName.trim() || !city.trim()) return Promise.resolve({ source: "local" });
  const cacheKey = `${placeName}::${city}`;
  const pending = inFlight.get(cacheKey);
  if (pending) return pending;

  const request = fetchTourInfo(placeName, city).finally(() => {
