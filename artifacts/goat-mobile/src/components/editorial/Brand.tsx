...
export function GoatMark({ light = false, compact = false }: { light?: boolean; compact?: boolean }) {
  ...
  return (
    <View accessible accessibilityRole="image" style={styles.row} accessibilityLabel="GOAT 강원 감성 여행">
      <View accessible={false} importantForAccessibility="no-hide-descendants"><GangwonSymbol size={compact ? 26 : 31} color={color} /></View>
...
export function GangwonSymbol({ size = 42, color = palette.forest }: { size?: number; color?: string }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 48 48" accessible={false} accessibilityElementsHidden>
