...
export function PlaceCard({ card, onPress }: PlaceCardProps) {
  ...
  const routeLabel = card.routeInfo
    ? [
        card.routeInfo.fromLabel,
        typeof card.routeInfo.durationMin === 'number' ? `약 ${card.routeInfo.durationMin}분` : null,
        card.routeInfo.distanceKm < 1 ? '1km 이내' : `약 ${Math.round(card.routeInfo.distanceKm)}km`,
        card.routeInfo.estimated ? '직선거리 추정' : null,
      ].filter(Boolean).join(' · ')
    : null;
    ...
    <View style={[styles.card, { backgroundColor: '#FAFAF9', borderColor: colors.border }]}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.infoRow}>
          ...
          {routeLabel && (
            <InfoPill icon="navigation" label={routeLabel} color={colors.mutedForeground} />
          )}
        </View>
        ...
        <View style={styles.actions}>
          <TouchableOpacity
            ...
            accessibilityLabel={`${place.place_name} 카카오맵에서 보기`}
          >
          ...
          <TouchableOpacity
            ...
            accessibilityLabel={`${place.place_name} 상세 보기`}
          >
...
const styles = StyleSheet.create({
  ...
  kakaoBtn: {
    ...
    minHeight: 44, backgroundColor: '#F7E600', paddingVertical: 11, paddingHorizontal: 16, borderRadius: 10,
  },
